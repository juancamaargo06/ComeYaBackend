...
export class UserController {
    ...
    async registerUser(request: Request, response: Response): Promise<Response> {
        ...
        try {
            ...
            const existingUser = await this.app.getUserByEmail(email);
            if (existingUser) {
    ...
    async updateUser(request: Request, response: Response): Promise<Response> {
        ...
        try {
            ...
            if (password && !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,25}$/.test(password)) {
                return response.status(400).json({
                    error: "La contraseña debe tener entre 6 y 25 caracteres, incluyendo al menos una letra y un número",
                });
            }

            const updatedUser = await this.app.updateUser(id, { name, email, password });
